...
```js
function getDescendants(nodes, id: number) {
  const childrenNodes = nodes.filter((n) => n.parent_id == id)
  if (childrenNodes.length == 0) return []
  return childrenNodes.concat(
    childrenNodes.map((node) => getDescendants(nodes, node.id)).flat()
  )
}
```
